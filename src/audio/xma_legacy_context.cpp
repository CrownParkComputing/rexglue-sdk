...
XmaLegacyContext::XmaLegacyContext()
...
int XmaLegacyContext::Setup(uint32_t id, memory::Memory* memory, uint32_t guest_ptr) {
  id_ = id;
  memory_ = memory;
...
void XmaLegacyContext::Enable() {
  std::lock_guard<std::mutex> lock(lock_);

  auto context_ptr = memory()->TranslateVirtual(guest_ptr());
  XMA_CONTEXT_DATA data(context_ptr);

  REXAPU_DEBUG("XmaContext: kicking context {} (buffer {} {}/{} bits)", id(),
...
void XmaLegacyContext::Disable() {
  std::lock_guard<std::mutex> lock(lock_);
  REXAPU_DEBUG("XmaContext: disabling context {}", id());
  set_is_enabled(false);
}

void XmaLegacyContext::Release() {
...
void XmaLegacyContext::SwapInputBuffer(XMA_CONTEXT_DATA* data) {
  // No more frames.
...
bool XmaLegacyContext::TrySetupNextLoop(XMA_CONTEXT_DATA* data,
...
*/
void XmaLegacyContext::NextPacket(XMA_CONTEXT_DATA* data) {
...
int XmaLegacyContext::GetSampleRate(int id) {
  switch (id) {
    case 0:
      return 24000;
    case 1:
...
bool XmaLegacyContext::ValidFrameOffset(uint8_t* block, size_t size_bytes,
                                     size_t frame_offset_bits) {
  uint32_t packet_num =
      GetFramePacketNumber(block, size_bytes, frame_offset_bits);
  if (packet_num == -1) {
    // Invalid packet number
    REXAPU_DEBUG("ValidFrameOffset: Invalid packet number");
    return false;
  }
  ...
  if (first_frame_offset == -1 || first_frame_offset > kBitsPerPacket) {
    REXAPU_DEBUG("ValidFrameOffset: Invalid frame offset {}", first_frame_offset);
    // Packet only contains a partial frame, so no frames can start here.
    return false;
  }

  stream::BitStream stream(packet, kBitsPerPacket);
  ...
  while (true) {
    ...
    if (stream.BitsRemaining() < 15) {
      REXAPU_DEBUG("ValidFrameOffset: No room for next frame header {}",
               first_frame_offset);
    ...
    if ((size - 15) > stream.BitsRemaining()) {
      REXAPU_DEBUG("ValidFrameOffset: Last frame {} - {}", first_frame_offset,
               size);
      // Last frame.
      return false;
    } else if (size == 0x7FFF) {
...
void XmaLegacyContext::Decode(XMA_CONTEXT_DATA* data) {
  ...
  if (!current_input_buffer) {
    REXAPU_ERROR("XmaContext {}: Error - input buffer pointer is invalid!", id());
    return;
  }

  if (!data->output_buffer_block_count) {
    REXAPU_ERROR("XmaContext {}: Error - Received 0 for output_buffer_block_count!",
           id());
    return;
  }
  ...
  while (output_remaining_bytes > 0) {
    ...
    stream::BitStream stream(current_input_buffer, current_input_size * 8);
    ...
    if (split_frame_len_) {
      ...
      if (split_frame_len_ > xma::kMaxFrameLength) {
        ...
        stream::BitStream slen(xma_frame_.data() + 1, 15 + split_frame_padding_start_);
      ...
      if (!ValidFrameOffset(current_input_buffer, current_input_size,
                            data->input_buffer_read_offset)) {
        REXAPU_DEBUG("XmaContext {}: Error - Invalid read offset {}!", id(),
                 static_cast<uint32_t>(data->input_buffer_read_offset));
        SwapInputBuffer(data);
        return;
      }
...

      {
        ...
        if (bits_to_copy < 0 || static_cast<size_t>(bits_to_copy) > stream.BitsRemaining()) {
          REXAPU_DEBUG(
    ...
    if (ret < 0) {
      REXAPU_ERROR("XmaContext {}: Error - Sending packet for decoding failed", id());
      // TODO bail out
      assert_always();
    }
    ...
    if (ret < 0) {
      ...
      REXAPU_ERROR("XmaContext {}: Error - Decoding failed: {} ({})", id(), errbuf,
             ret);
...

    {
      ...
      XmaContext::ConvertFrame(reinterpret_cast<const uint8_t**>(&av_frame_->data),
                   bool(av_frame_->channels > 1),
                   raw_frame_.data());
...
uint32_t XmaLegacyContext::GetPacketFirstFrameOffset(
...
size_t XmaLegacyContext::GetNextFrame(uint8_t* block, size_t size,
                                   size_t bit_offset) {
  ...
  stream::BitStream stream(block, size * 8);
...
int XmaLegacyContext::GetFramePacketNumber(uint8_t* block, size_t size,
                                        size_t bit_offset) {
  size *= 8;
  if (bit_offset >= size) {
    // Not good :(
...
std::tuple<int, int> XmaLegacyContext::GetFrameNumber(uint8_t* block, size_t size,
                                                   size_t bit_offset) {
  auto packet_idx = GetFramePacketNumber(block, size, bit_offset);
...
std::tuple<int, bool> XmaLegacyContext::GetPacketFrameCount(uint8_t* packet) {
  auto first_frame_offset = xma::GetPacketFrameOffset(packet);
...
int XmaLegacyContext::PrepareDecoder(uint8_t* packet, int sample_rate,
                                  bool is_two_channel) {
  ...
  if (av_context_->sample_rate != sample_rate ||
      av_context_->channels != (int)channels) {
