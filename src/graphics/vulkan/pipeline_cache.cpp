...
// GetGeometryShaderKey lives in geometry_shader_builder.cpp alongside the
// builder it keys, so the native GPU backend compiles both without the rest
// of this cache.
...
VkShaderModule VulkanPipelineCache::GetGeometryShader(GeometryShaderKey key) {
  ...
  // The builder itself lives in geometry_shader_builder.cpp so the native GPU
  // backend can compile it without the rest of this cache. Store the handle
  // even if creation failed, so it is not retried every draw.
  VkShaderModule shader_module =
